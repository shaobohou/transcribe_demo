...
def test_cli_main_reads_api_key_from_environment(monkeypatch):
    ...
    monkeypatch.setattr("sys.argv", ["transcribe-demo", "--config.backend=realtime"])
...
def test_cli_main_prefers_explicit_api_key_over_env(monkeypatch):
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "env-test-key")
    monkeypatch.setattr(
        "sys.argv", ["transcribe-demo", "--config.backend=realtime", "--config.realtime.api_key=explicit-key"]
    )
